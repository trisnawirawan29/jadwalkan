            ...
            <div class="content-card booking-detail-card">
                ...
                @if ($booking->status === 'held' || $booking->status === 'rejected')
                    @php
                        ...
                        $paymentProvider = $paymentPlace->provider;
                        $bankPaymentAvailable = $paymentPlace->bank_payment_enabled && $paymentProvider?->payment_bank_name && $paymentProvider?->payment_bank_account_number;
                        $qrisPaymentAvailable = $paymentPlace->qris_payment_enabled && $paymentProvider?->payment_qris_image_url;
                    @endphp
                    @if ($bankPaymentAvailable || $qrisPaymentAvailable)
                        <div class="payment-instruction-card mt-4"><div class="d-flex align-items-start gap-3"><span class="payment-instruction-icon"><i class="fas fa-wallet"></i></span><div><strong>{{ __('Informasi pembayaran') }}</strong><p>{{ __('Gunakan salah satu metode pembayaran berikut, lalu unggah bukti pembayaran Anda.') }}</p></div></div><div class="payment-method-cards mt-3">@if ($bankPaymentAvailable)<div class="payment-method-card payment-method-card-bank"><div class="payment-method-card-heading"><span class="payment-method-card-icon"><i class="fas fa-building-columns"></i></span><div><strong>{{ __('Transfer bank') }}</strong><small>{{ $paymentProvider->payment_bank_name }}</small></div></div><div class="payment-account-number">{{ $paymentProvider->payment_bank_account_number }}</div><small class="payment-account-name">{{ __('a.n.') }} {{ $paymentProvider->payment_bank_account_name ?: '-' }}</small></div>@endif @if ($qrisPaymentAvailable)<div class="payment-method-card payment-method-card-qris"><div class="payment-method-card-heading"><span class="payment-method-card-icon"><i class="fas fa-qrcode"></i></span><div><strong>{{ __('Bayar dengan QRIS') }}</strong><small>{{ __('Scan kode untuk membayar') }}</small></div></div><img src="{{ $paymentProvider->payment_qris_image_url }}" alt="QRIS {{ $paymentPlace->name }}"></div>@endif</div><div class="payment-proof-tip mt-3"><i class="fas fa-camera"></i><span>{{ __('Setelah transfer, screenshot atau foto bukti pembayaran Anda. Kirim melalui tombol konfirmasi pembayaran di bawah.') }}</span></div></div>
                    @endif
