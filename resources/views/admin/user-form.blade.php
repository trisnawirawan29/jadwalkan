...
@section('content')
    ...
    <div class="content-card col-12 col-xl-8">
        ...
        <form method="POST" action="{{ $user->exists ? route('admin.users.update', $user) : route('admin.users.store') }}">
            ...
            <div class="mb-3"><label class="form-label" for="provider_plan_id">Paket provider <span class="text-muted">(opsional)</span></label><select id="provider_plan_id" name="provider_plan_id" class="form-select"><option value="">Tanpa paket</option>@foreach ($providerPlans as $providerPlan)<option value="{{ $providerPlan->id }}" @selected((string) old('provider_plan_id', $user->provider_plan_id) === (string) $providerPlan->id)>{{ $providerPlan->name }} · Rp {{ number_format((float) $providerPlan->monthly_price, 0, ',', '.') }}/bulan</option>@endforeach</select><small class="text-muted">Paket menentukan jumlah bisnis, layanan per bisnis, dan booking bulanan provider.</small></div>
